...
Even more compact, we can distill the notation above into a single matrix equation for the individual $s$'s predicted scores on all $y$ variables:
...
Note that our model-predicted scores on $y$ for a given individual $s$ do not have an expectation of the value when the factor scores $\eta$ are zero. This corresponds to the default 'vanilla' SEM where the means are not explicitly part of the model representation. Rather, the model is only tasked with explaining the sample variance-covariance matrix, $\mathbf{S_{XX}}$.

As we'll return to later, we can add mean structure to the model, typically annotated as $\boldsymbol{\tau}$.

$$
\begin{align*}
\textrm{No means:} \quad & y_s = \boldsymbol{\Lambda}_y \eta_s + \varepsilon_s \\
\textrm{Means:} \quad & y_s = \tau_y + \boldsymbol{\Lambda}_y \eta_s + \varepsilon_s \\
\textrm{Regression:} \quad & y_s = \beta_0 + \beta_1 x_s + \varepsilon_s
\end{align*}
$$
