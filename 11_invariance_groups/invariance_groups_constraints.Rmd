...
1.  What is the mean difference in depression -- measuremed by a multi-item factor -- before and after cognitive behavior therapy?
2.  How does the mean leve of conscientiousness (in a Big 5 sense) change over the life span in the population?
...
In the depression example, notice how we are leveraging the power of SEM by specifying a measurement model for depression, we handle measurement error formally when testing for mean change over time. This should both increase the precision of our estimate and reduce bias. This holds true for the conscientiousness question, too.
...
Note that we use the triangle with a 1 inside to denote the regression of a variable (e.g., the factor scores) on a constant (1). 
...
msem.syntax <- '
l_FoGWar =~ NA*FoGWar1 + f1*FoGWar1 + f2*FoGWar2 + f3*FoGWar3 + f4*FoGWar4  #definition of factor FoGWar with loadings on 4 items
l_CoAWe =~ NA*CoAWe5 + CoAWe6 + CoAWe7 + CoAWe8  #definition of factor CoAWe with loadings on 4 items
...
The tau-equivalent model fits significantly worse than the congeneric model. Moreover, the AIC difference is ~44.3 points, consistent with large differences in model evidence.

Overall, we need to attend to global fit indices beyond NHST alone to judge constrained models. One should make an informed judgment based on a) LRT, b) changes in global fit measures (esp. change in CFI), c) change in residuals under the constrained solution, and d) interpretability.
...
In the parallel model, we add constraints on disturbances for the indicators (error variance). Let's apply these constraints to the tau-equivalent CoAWe model, leaving FoGWar as congeneric. Remember that we only consider the parallel model if the tau-equivalent step fits well. (So here, we should have stopped above -- just continuing for didactic value.)
